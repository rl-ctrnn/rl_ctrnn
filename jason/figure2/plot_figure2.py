...
data = np.genfromtxt(f"figure2_{learning_duration/1000}k_initflux-{initflux}.dat",delimiter=",", dtype=float)
...
for index in range(len(data)):
    ...
    if final_fit>threshold:
        ...
        alpha=0.1    
    plt.plot( range(learning_duration-start), data[index][start+5:], color=alpha_adj(cmap(improvement) ) )
